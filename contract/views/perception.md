...
本文只列 Perception 域**产出**与**消费**的跨域 endpoint，用于分域阅读。
...
| --- | --- | --- | --- | --- | --- |
| P-01 | `/perception/build_wall_task_plan` | Action | `robot_task_interfaces/action/BuildWallTaskPlan` | Autonomy | — |
| P-02 | `/perception/refine_sequence_poses` | Action | `robot_task_interfaces/action/RefineSequencePoses` | Autonomy | — |
...
| P-NAV-02 | `/tf` | Topic | `tf2_msgs/msg/TFMessage` | RT-Control、Motion、Autonomy | ROS 标准类型；map → odom，唯一发布者为 Perception |
| N-05 | `/navigation/localization/status` | Topic | `robot_navigation_interfaces/msg/LocalizationStatus` | Motion、Autonomy | Q_STATE；10～20 Hz；最大年龄 200 ms |
